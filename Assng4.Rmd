...
confusionMatrix(table(predictions, as.vector(tumor_test$status)))

```
The model was accurate in its classification 92.94 % of the time. Overall, the results of elastic net indicate that ridge regression be maintained to maximize prediction accuracy.
...
# Create CM
# k=3
confusion_matrix_all <- table(knn.pred_all, test_status)

# Calculate accuracy, sensitivity, specificity after using quadratic effects
accuracy_sq <- (sum(diag(confusion_matrix_all)) / sum(confusion_matrix_all))*100
sensitivity_sq <- (confusion_matrix_all[2,2]/ (confusion_matrix_all[2,1] + confusion_matrix_all[2,2]))*100
specificity_sq <- (confusion_matrix_all[1,1] / (confusion_matrix_all[1,1] + confusion_matrix_all[1,2])) * 100
...
for (i in 1:ncol(train.X_all)) {
  ...
  accuracy_temp <- (sum(diag(confusion_matrix_temp)) / sum(confusion_matrix_temp))*100
...
tumor_train <- read.csv("tumor_train.csv")
tumor_test <- read.csv("tumor_test.csv")
...
conf_mat <- table(ypred, testdat$y)
accuracy <- (sum(diag(conf_mat)) / sum(conf_mat))*100
sensitivity <- (conf_mat[2,2]/ (conf_mat[2,1] + conf_mat[2,2]))*100
specificity <- (conf_mat[1,1] / (conf_mat[1,1] + conf_mat[1,2])) * 100
...
conf_mat2 <- table(ypred2, testdat2$y)
accuracy2 <- (sum(diag(conf_mat2)) / sum(conf_mat2))*100
sensitivity2 <- (conf_mat2[2,2]/ (conf_mat2[2,1] + conf_mat2[2,2]))*100
specificity2 <- (conf_mat2[1,1] / (conf_mat2[1,1] + conf_mat2[1,2])) * 100
...
for (i in 1:ncol(dat2[, 2:31])) {
  ...
  conf_mat2_temp <- table(ypred2_temp, testdat2$y)
  accuracy2_temp <- (sum(diag(conf_mat2_temp)) / sum(conf_mat2_temp)) * 100
