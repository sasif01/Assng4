...
# combine the predictor matrix and target feature into a data frame without row names
dat <- data.frame(x = x, y = as.factor(y), row.names = NULL)
...
View(testset)
# combine the predictor matrix and target feature into a data frame without row names
testdat <- data.frame(x = xtest, y = as.factor(ytest), row.names = NULL)
