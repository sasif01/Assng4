...
# Variance is different across predictors, so it has not been standerdized, however glmnet automatically standardizes the variables
apply(xs, 2, var)
...
#cv.glmnet does NOT search for values for alpha, default value is 1 (lasso)
def_elnet = train(status ~ ., data=tumor_train, method = "glmnet", trControl = enet)
def_elnet$results
#perform k-fold cross-validation to find optimal lambda value that minimizes PE/MSE
cv_model <- cv.glmnet(xs, status, type.measure = "class")

prd.fl <- predict(cv_model, newx = tumor_test[,-1], s = c(cv_model$lambda.min, cv_model$lambda.1se)) 
